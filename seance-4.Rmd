...
bibliothèques du `tidyverse` et en particulier `ggplot2` pour explorer des 
données spatiales à l'échelle d'un site. Nous utiliserons le même jeu de données 
que dans les séances précédentes.  
Préparons un peu notre script en ouvrant des bibliothèques 
```{r library, echo=F}
#library(dplyr)
#library(ggplot2)
# ou plus simplement avec la commande suivante, avec laquelle toutes les 
# bibliothèques du tidyverse peuvent être chargées en une fois :
library(tidyverse)
```
...
2) d'avoir une unité graphique dans votre document en utilisant toujours le même "layout" (agencement de paramètres);   
...
Les coordonnées sont en cm: en x elles se situent entre 10 et 290 cm, et en y entre 50 et 390 cm. Avec un carroyage basé sur des carrés de 1 m de côté, les noeuds auront pour coordonnées (0;0), (0;100), (0;200), etc. Allons-y avec `ggplot2` !  
...
plan <- ggplot2::ggplot() +
  ...
  # de 0 à 400 cm, puis en subdivisant les axes tous les 1 m 
  scale_x_continuous(breaks = seq(0, 400, by = 100)) + 
  scale_y_continuous(breaks = seq(0, 400, by = 100)) + 
  coord_fixed(ratio = 1) + # même échelle en x et en y (ratio = 1), 
  ylab(element_blank()) +  # enfin, ici on définit les labels des axes 
  xlab(element_blank()) +
  annotate (geom = "text", 
            x = c(seq(from = 50, to = 400, by = 100)), 
            y = -10, 
            label = carres.x) +
  annotate (geom = "text",
            x = -10, 
            y = c(seq(from = 50, to = 400, by = 100)), 
            label = carres.y)
plan  
```
...
plan +
  ...
  labs(title = "Répartition spatiale des artefacts - site de la Forêt")
```
...
plan +
  geom_point(data = jdd, 
             mapping = aes(x = x_cm, y = y_cm, 
                           color = silex, 
                           shape = type, 
                           size = 1)) + # on augmente un peu la taille des points 
                                        # pour plus de lisibilité
  labs(title = "Répartition spatiale des artefacts - site de la Forêt")
```

La palette automatique ne contient pas assez de formes (voir message d'erreur). 
Nous allons donc devoir les définir manuellement dans les esthétiques de la figure. 
Pour cela nous allons attribuer à chaque valeur de `type` une forme.  

Affichons pour commencer les levels de `type` (après l'avoir converti en factor).
```{r type factor, echo=T}
jdd$type_fac <- factor(
  jdd$type, 
  # les levels sont dans l'ordre alphabétique ; 
  # il est possible de changer leur ordre si on le souhaite
...
On créé ensuite manuellement la palette de formes avec la fonction
`scale_shape_manual()`
...
On peut aussi faire la même chose pour la palette de couleurs. Utilisons ici les couleurs de la palette `viridis`. 

```{r voir viridis, echo=T}
#pour voir la palette on peut utiliser les bibliothèques `pals` et `scales`
#install.packages("pals","scales")
library(pals)
library(scales)
# et la fonction show_col()
show_col(viridis(24))

# la fonction viridis() permet d'afficher les codes HEX des couleurs qui
#composent la palette
viridis(n=24)
```
...
plan +
  ...
  scale_shape_manual(values = c("nucleus a eclats laminaires" = 7, 
                                ...
                                "burin diedre" = 24, "percoir" = 10,
                                "picardie" = 14, "gravette" = 3)) +
  scale_color_manual(values = c("type A"="#7AD151FF", 
                                "type B"="#2A788EFF", 
                                "type C"="#440154FF")) +
  scale_fill_manual(values = c("type A"="#7AD151FF", 
                               "type B"="#2A788EFF", 
                               "type C"="#440154FF")) +
  labs(title = "Répartition spatiale des artefacts - site de la Forêt")
```
...
# choisissez vos couleurs et créez votre palette personnalisée
viridis(n=3)

shape <- c("nucleus a eclats laminaires" = 7, "burin du Raysse" = 12, 
           "eclat" = 22, "lame" = 23, "chute de burin" = 21, "burin" = 8, 
           "burin diedre" = 24, "percoir" = 10, "picardie" = 14, 
           "gravette" = 3)
cols <- c("type A"="#7AD151FF", "type B"="#2A788EFF", "type C"="#440154FF")
```

Pour alléger encore, on place les palettes et le titre dans l'objet `plan.modif`. 
```{r plan modif, echo=T}
plan.modif <- plan +
...
Les connecteurs logiques permettent de personnaliser vos sélections très facilement. Voici les différentes possibilités : 
...
**!=** : "non égal à"  
**!x** : "différent de x"  
**isTRUE(x)** : teste si x est TRUE  

## - Quelques exemples !
On sélectionne les pièces qui ne sont pas en silex de type A 
```{r select silex, echo=T}
jdd.silexBC <- subset(jdd, silex != "type A",)

plan.modif +
  geom_point(data = jdd.silexBC, mapping = aes(x = x_cm, y = y_cm, color = silex, 
                                               fill = silex, shape = type_fac, size = 1)) +
  # titre et légende
...
```{r select type, echo=T}
jdd.lamn <- subset(jdd, type == "eclat" | type == "lame" | type == "nucleus a eclats laminaires",)

plan.modif +
  geom_point(data = jdd.lamn, mapping = aes(x = x_cm, y = y_cm, color = silex, 
                                             fill = silex, shape = type_fac, size = 1)) +
  # titre et légende
...
Mettons en pratique ce concept sur le jeu de données du site de la Forêt. Nous allons créer plusieurs plans de répartition en fonction de la dimensions des pièces. Tout d'abord, rafraîchissons nous la mémoire sur les statistiques basiques du jeu de données.
```{r summary 2, echo=T}
summary(jdd)
```
