...
Bien commenter son code est très important car cela nous permet de le comprendre facilement même après des mois et des années sans y avoir travaillé (en théorie !).
...
Lorsque l'on ne comprends pas l'objectif d'une fonction ou que l'on a oublié, on peut demander de l'aide avec la fonction `help()` ou plus simplement à `?class`. 
```{r help, echo=T}
?class
...
Pour vérifier le type de variable dans un tableau (ce sera important car certaines fonctions qui nécessitent un type spécifique de variable), on peut appliquer la fonction `class()` sur chaque variable que l'on associe au jeu de donné avec l'opérateur `$`
```{r classcol, echo=T}
class(jdd$type)
class(jdd$longueur)
```
...
Un argument est un objet que l'on passera à la fonction pour qu'elle s'éxecute. De faÃ§on générale, les arguments sont rentrés dans l'ordre chronologique dans lequel ils sont programmés dans la fonction. Voyons voir cela de plus près. 
```{r plothelp, echo=T}
?plot
```
...
Notez ici l'importance de bien séparer chaque argument par une virgule `,`. Sans cela R il n'est pas possible de passer la commande.  
...
Les points de ce graphe sont trop petits pour être bien visibles. Mais bien sÃ»r il est possible de modifier leur taille au sein de la fonction `plot()`. 
```{r plot6, echo=T}
plot(x = jdd$longueur, y = jdd$largeur,
     col = jdd$type_fac, 
...
### et on clÃ´ture l'écriture de la figure dans le pdf
