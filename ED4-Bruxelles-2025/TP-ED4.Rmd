...
permettrons de décrire les étapes du script. Bien commenter son code est
...
allons donc convertir la variable "type" en facteur avec la fonction
factor : 
...
rendre d'erreurs à un stade préliminaire. 
...
largeur des pièces. Dans le R de base, on utilise la fonction `plot()`. 
...
automatiquement considérés comme des objets de type caractère. Il est
...
plot(x = jdd$longueur, y = jdd$largeur, xlab = xlabel, ylab = ylabel)
...
notamment de nombreux packages qui codent des palettes prédéfinies,
...
personnalisées : la méthode "basique" dans R, et l'emploi d'un package
déjà codé. 
...
Tout d'abord, avec la fonction `length()`, je détermine le nombre de
...
Il est possible d'ajouter plusieurs légendes sur une même figure ! Il suffit d'ajouter une 
...
figures similaires de la méme taille exactement ! Cette façon de faire
est plus reproductible et ne complique par le code outre mesure. 

Enregistrons notre figure en format pdf. Il faut tout d'abord créer un
fichier pdf vide de la taille voulue : 

```{r pdf, eval=FALSE, echo=TRUE}

pdf("Figure1.pdf", # ici on écrit le chemin vers la figure 
