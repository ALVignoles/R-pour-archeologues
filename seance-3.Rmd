---
title: "Séance 3 : Sous-ensembles et boucles de programmation"
output: bookdown::html_document2
---

# - Introduction
Au cours de cette séance, nous allons continuer à nous familiariser avec les packages du `tidyverse` et `ggplot2` pour explorer des données spatiales à l'échelle d'un site. Nous utiliserons le même jeu de données que dans la séance 1.  
...
1) de gagner un temps précieux, le code ne prenant que quelques sechondes pour générer une figure ;  
...
plan <- ggplot2::ggplot() +
  ...
  theme(axis.ticks = element_blank(), 
    ...
    # ici, on définit les dimensions des axes : de 0 à 4 cm, avec la même échelle en x et en y (ratio = 1)
...
plan.modif +
  ...
  labs(color = "Matière première", shape = "Catégorie d'objet") +
    # et là, nous pouvons supprimer les légendes que nous ne souhaitons pas voir apparaÃ®tre dans la figure finale. 
...
Mettons en pratique ce concept sur le jeu de données du site de la Forêt. Nous allons créer plusieurs plans de Répartition en fonction de la dimensions des pièces. Tout d'abord, rafraÃ®chissons nous la mémoire sur les statistiques basiques du jeu de données.
```{r summary 2, echo=T}
summary(jdd)
```
