...
plan.modif +
  geom_point(data = jdd.lamn, mapping = aes(x = x..cm., y = y..cm., color = silex, 
                                             fill = silex, shape = type_fac, size = 1)) +
  # titre et légende
  labs(title = "Répartition spatiale des pièces non retouchées pouvant s'intégrer dans un schéma laminaire",
       color = "Matière première", shape = "Catégorie d'objet") +
  guides(fill = "none", size = "none")
...
## afficher une valeur tant que celle-ci est inférieure à 5.
...
# des pièces. Tout d'abord, rafraîchissons nous la mémoire sur les statistiques
...
### à mesure. 
dir.create("Output/plans_fct_longueur")
...
for(i in 1:11) { # Cette boucle va s'exécuter tant que i sera compris entre 1 et 11
  ...
  plan.modif +
    ...
    labs(title = paste0("Répartition spatiale des pièces dont L est comprise entre ", borne.min,  " et ", borne.max, "cm"),
         ...
         ### Cette fonction est extrêmement utile, retenez là ! Elle permet de coller côte à côte des objets en les
  ...
  ggsave(paste("Output/plans_fct_longueur/répart_pièces_entre ", borne.min, "et", borne.max, ".png", sep = "_"), 
         width = 6, height = 4)
}
